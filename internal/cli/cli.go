...
func status(ctx context.Context, app *frontend.App, out io.Writer, args []string) error {
	...
	// A pause has the same CONSEQUENCE as isolation and a completely different
	// remedy, so it says both: what is not happening, and the one key that
	// ends it. Not a warning — the operator asked for this.
...
func configCmd(ctx context.Context, app *frontend.App, out io.Writer, args []string) error {
	...
	case "set":
		if len(args) < 3 {
			return fmt.Errorf("usage: config set <field> <value>, or config set <command field> --preset <%s> (see: config fields)",
				strings.Join(frontend.LLMPresetNames, "|"))
		}
...
func printConfig(out io.Writer, cfg config.Config) {
	fmt.Fprintf(out, "confidence thresholds: minimum=%.2f idle=%.2f approval=%.2f choice=%.2f error=%.2f\n",
...
func taskSource(ctx context.Context, app *frontend.App, out io.Writer, args []string) error {
	if len(args) > 0 && args[0] == "list" {
		...
		for i, src := range cfg.TaskSources {
			...
			// listing. Resolved through the accessor — the field is a *bool,
			// and %v on it would print a pointer address.
			fmt.Fprintf(out, " enable_llm_review_before_auto_send=%v", src.ReviewBeforeAutoSendEnabled())
...
func taskSourceSet(ctx context.Context, app *frontend.App, out io.Writer, args []string) error {
	...
	case "enable-llm-review-before-auto-send", "enable_llm_review_before_auto_send":
		...
		if err := app.SetTaskSourceReviewBeforeAutoSend(ctx, idx, expected, on); err != nil {
			return err
		}
	...
	case "enable-llm-review", "enable_llm_review", "llm-review", "llm_review":
		...
		// migrates `enable_llm_review` away and warns about it, so accepting it
		// here would teach a spelling hap is retiring.
		return fmt.Errorf("task-source key %q was renamed — use enable-llm-review-before-auto-send\n%s", key, usage)
