...
const historicalCodexMCQ = `Question 1/3 (3 unanswered)
...
func TestAuditReclassifyCodexMCQPreviewAndApply(t *testing.T) {
	app, st := testApp(t)
	ctx := context.Background()
	created := time.Unix(1_700_000_000, 0)
	id, err := st.AppendAudit(ctx, domain.AuditRecord{
		AgentID: "pane-393", AgentType: "Codex", Signature: "unclassifiable:old393",
		Trigger: "agent-status: blocked", SituationType: domain.SituationUnclassifiable,
		Action: "escalated", Input: "kept input", Rationale: "[unclassifiable] no known prompt",
		Status: "resolved", Suggestion: "kept suggestion", PaneExcerpt: historicalCodexMCQ,
		CreatedAt: created,
	})
	if err != nil {
		t.Fatal(err)
	}
	// SQL candidates that are not structurally Codex forms, plus other agents
	// and already-classified rows, must never appear in the preview.
	for _, rec := range []domain.AuditRecord{
		{AgentType: "codex", Signature: "unclassifiable:narration", SituationType: domain.SituationUnclassifiable, PaneExcerpt: "ordinary narration", Status: "escalated", CreatedAt: created},
		{AgentType: "claude", Signature: "unclassifiable:claude", SituationType: domain.SituationUnclassifiable, PaneExcerpt: historicalCodexMCQ, Status: "escalated", CreatedAt: created},
		{AgentType: "codex", Signature: "choice:existing", SituationType: domain.SituationChoice, PaneExcerpt: historicalCodexMCQ, Status: "escalated", CreatedAt: created},
	} {
	...
	out, err := run(t, app, "audit", "reclassify-codex-mcq")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out, fmt.Sprintf("#%d\twould reclassify", id)) ||
		!strings.Contains(out, "questions=3") || !strings.Contains(out, "dry run only") {
		t.Fatalf("preview output:\n%s", out)
	}
	if strings.Contains(out, "narration") || strings.Count(out, "would reclassify") != 1 {
		t.Fatalf("preview included an ineligible row:\n%s", out)
	}
	before, _ := st.GetAudit(ctx, id)
	if before.SituationType != domain.SituationUnclassifiable || before.Signature != "unclassifiable:old393" {
		t.Fatalf("dry run mutated row: %+v", before)
	}

	out, err = run(t, app, "audit", "reclassify-codex-mcq", "--id", fmt.Sprint(id), "--apply")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(out, fmt.Sprintf("#%d\treclassified", id)) || !strings.Contains(out, "reclassified 1 of 1") {
		t.Fatalf("apply output:\n%s", out)
	}
	after, _ := st.GetAudit(ctx, id)
	if after.SituationType != domain.SituationChoice || !strings.HasPrefix(after.Signature, "choice:") {
		t.Fatalf("classification was not repaired: %+v", after)
	}
	if after.Status != "resolved" || after.Action != "escalated" || after.Input != "kept input" || after.Suggestion != "kept suggestion" {
		t.Fatalf("repair changed operator/status fields: %+v", after)
	}
	if after.CreatedAt != created || !strings.Contains(after.Rationale, "historical reclassification") || !strings.Contains(after.Rationale, "[unclassifiable]") {
		t.Fatalf("repair lost audit provenance: %+v", after)
	}
	snapshot, err := st.GetSignatureSnapshot(ctx, after.Signature)
	if err != nil || snapshot != historicalCodexMCQ {
		t.Fatalf("new signature snapshot = %q, %v", snapshot, err)
	}

	if _, err := run(t, app, "audit", "reclassify-codex-mcq", "--id", fmt.Sprint(id), "--apply"); err == nil ||
		!strings.Contains(err.Error(), "not an unclassifiable Codex MCQ") {
		t.Fatalf("second ID-scoped apply should be safely ineligible, got %v", err)
	}
}

func TestAuditReclassifyCodexMCQEmptyAndValidation(t *testing.T) {
	app, _ := testApp(t)
	out, err := run(t, app, "audit", "reclassify-codex-mcq")
	if err != nil || !strings.Contains(out, "no unclassifiable Codex MCQ") {
		t.Fatalf("empty preview: out=%q err=%v", out, err)
	}
	for _, args := range [][]string{{"--id", "-1"}, {"unexpected"}} {
		if _, err := run(t, app, "audit", append([]string{"reclassify-codex-mcq"}, args...)...); err == nil {
