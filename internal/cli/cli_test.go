...
// TestTaskSourceListShowsLLMReview: unlike auto_send_when_idle (printed only
// when true), the review state is ALWAYS printed — false is the state that
// changes behavior silently, and it is what an upgraded config becomes.
func TestTaskSourceListShowsLLMReview(t *testing.T) {
	...
	if strings.Contains(out, "0x") {
		t.Errorf("listing leaked a pointer address:\n%s", out)
	}
}

// TestTaskSourceAddLLMReview pins the add flag, in the positions Go's flag
// package accepts — and the ordering guard for the position it does not.
func TestTaskSourceAddLLMReview(t *testing.T) {
	for _, tc := range []struct {
		name    string
		args    []string // "PATH" is replaced by the checklist path
		want    bool
		wantErr bool
	}{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			args := append([]string{"task-source"}, tc.args...)
			for i, a := range args {
				if a == "PATH" {
					args[i] = path
				}
