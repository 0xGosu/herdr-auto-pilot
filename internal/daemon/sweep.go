...
// picker autonomously through the shared keyed-form body (deliverKeyedForm):
// audit-first, then the adaptive mcqdeliver protocol off the main loop. The
// deliverer re-reads the live pane itself and fails closed when the picker is
// gone or the learned label matches none of the offered environments (its
// approval signature is global, so a rule learned on another project's
// environment list can resolve here).
func (d *Daemon) deliverRemoteEnv(ctx context.Context, ks ports.KeystrokeSender,
	...
	d.deliverKeyedForm(ctx, d.remoteEnvForm(ks, s.PaneID), s, sig, dec, tr, now)
}
...
func (d *Daemon) deliverRemoteEnvLLM(ctx context.Context, ks ports.KeystrokeSender,
	...
	d.deliverKeyedFormLLM(ctx, d.remoteEnvForm(ks, s.PaneID), s, sig, tr, llmDec, confidence, llmConfidence, now)
}
