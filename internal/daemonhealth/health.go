...
// EngineLibSQL is the libsql engine's name in FleetSyncHealth.Engine. It
// reports like turso — a local replica with unpushed changes that can be
// isolated from the fleet while still serving this node — so only its
// bootstrap remedy differs (frontend.DaemonHealth).
