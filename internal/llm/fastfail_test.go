...
// command_start, rewrite_command / rewrite_command_start) exits with an error
// well inside fastFailWindow, the adapter retries once with the alternate
// template. The canonical trigger is claude's `--resume <session>` rejecting a
...
func TestRewriteFastFailRetriesAlternate(t *testing.T) {
	tests := []struct {
		name          string
		first         bool
		failMarker    string
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			logFile := filepath.Join(dir, "log")
			script := writeScript(t, fmt.Sprintf(
				"echo \"$1\" >> %s\nif [ \"$1\" = %q ]; then echo \"rewritten-$1\"; exit 0; fi\necho \"boom-$1\" 1>&2\nexit 1\n",
				logFile, tc.successMarker))
			a := &Adapter{
				RewriteTemplate:      []string{script, "base"},
				RewriteStartTemplate: []string{script, "start"},
				RewriteTimeout:       5 * time.Second,
			}
			got, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi", First: tc.first})
			if err != nil {
				t.Fatalf("fast fail should retry and succeed: %v", err)
			}
			if want := "rewritten-" + tc.successMarker; got != want {
...
func TestRewriteFastFailBothFailSurfacesPrimaryError(t *testing.T) {
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	script := writeScript(t, fmt.Sprintf(
		"echo \"$1\" >> %s\necho \"boom-$1\" 1>&2\nexit 1\n", logFile))
	...
	_, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi", First: true})
	if err == nil {
		t.Fatal("both-fail must error")
	}
	if !strings.Contains(err.Error(), "boom-start") {
...
func TestRewriteSlowFailDoesNotRetry(t *testing.T) {
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	script := writeScript(t, fmt.Sprintf("echo \"$1\" >> %s\nsleep 1.3\nexit 1\n", logFile))
	a := &Adapter{
	...
	if _, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi", First: true}); err == nil {
		t.Fatal("slow fail still errors")
	}
	if got := markers(t, logFile); !slices.Equal(got, []string{"start"}) {
		t.Errorf("slow fail must not retry, ran: %v", got)
...
func TestRewriteFastFailNoAlternateNoRetry(t *testing.T) {
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	script := writeScript(t, fmt.Sprintf("echo \"$1\" >> %s\nexit 1\n", logFile))
	a := &Adapter{
	...
	if _, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi"}); err == nil {
		t.Fatal("must error")
	}
	if got := markers(t, logFile); !slices.Equal(got, []string{"base"}) {
		t.Errorf("no alternate → single run, ran: %v", got)
...
func TestRewriteFastFailIdenticalTemplatesNoRetry(t *testing.T) {
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	script := writeScript(t, fmt.Sprintf("echo ran >> %s\nexit 1\n", logFile))
	tmpl := []string{script, "base"}
	a := &Adapter{
		RewriteTemplate:      tmpl,
		RewriteStartTemplate: tmpl,
		RewriteTimeout:       5 * time.Second,
	}
	if _, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi", First: true}); err == nil {
		t.Fatal("must error")
...
func TestRewriteTimeoutDoesNotRetry(t *testing.T) {
	// A true timeout (DeadlineExceeded) is not a fast fail: no retry, and the
	// error is worded as a timeout.
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	...
	a := &Adapter{
		RewriteTemplate:      []string{script, "base"},
		RewriteStartTemplate: []string{script, "start"},
		RewriteTimeout:       200 * time.Millisecond,
	}
	_, err := a.Rewrite(context.Background(), domain.RewriteRequest{Text: "hi", First: true})
	if err == nil || !strings.Contains(err.Error(), "timeout") {
		t.Fatalf("timeout must error with a timeout message, got %v", err)
...
func TestRewriteCancelledContextNoRetry(t *testing.T) {
	// A cancelled parent context makes every run "fail fast"; the retry guard
	// (ctx.Err() == nil) must suppress a pointless second spawn.
	dir := t.TempDir()
	logFile := filepath.Join(dir, "log")
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := a.Rewrite(ctx, domain.RewriteRequest{Text: "hi", First: true}); err == nil {
		t.Fatal("cancelled rewrite errors")
	}
	if got := markers(t, logFile); len(got) > 1 {
		t.Errorf("cancelled context must not retry, ran: %v", got)
	}
