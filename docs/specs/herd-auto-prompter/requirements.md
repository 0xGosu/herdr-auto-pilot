...
*Acceptance:* A signature the operator resolved the same way 9 of the last 10 times yields high confidence; an evenly split history yields low confidence; recent decisions shift the score more than older ones.
...
**FR-007 — Regression to shadow mode.**
If the operator corrects an autonomous decision (post-hoc or live), the plugin SHALL demote the affected signature back toward shadow mode by **resetting its consecutive-confirmation count to zero**. The recency-weighted agreement ratio (FR-005) is **not additionally discounted** by the correction beyond the effect of recording the corrected decision itself; consequently a demoted signature MUST re-earn N consecutive consistent confirmations (FR-006) before it can act autonomously again, even though its residual confidence score may remain high.
*Acceptance:* After a correction to an autonomous decision, the same signature is in shadow mode with a consecutive-confirmation count of zero and requires N fresh consecutive confirmations before re-graduating; the correction is itself recorded as a decision in the recency window.
