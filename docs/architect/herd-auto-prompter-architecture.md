...
| *(own file)* **`<state>/orchestrator-events.db`** | NOT a store table: the machine-local, append-only event log behind `hap stream orchestrator` (`internal/streamlog`). AUTOINCREMENT seq is the resume cursor; every hap process on the machine appends best-effort after its change commits; 7-day retention pruned by the daemon |
