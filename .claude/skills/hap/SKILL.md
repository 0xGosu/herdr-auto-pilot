...
| `embedding.bm25_min_score` | 0.35 | min normalized BM25 similarity for the text fallback. the fallback runs whenever vector search did not match — the embedder being unavailable or errored, AND a search that ran cleanly but found nothing above `similarity_threshold`. so this is also the bar deciding whether a screen cosine judged too dissimilar is still a textual near-duplicate of a learned rule |
